...
async function main() {
    ...
    function loadWeekMarkdown(weekNumber, elementClass) {
        fetch(`https://raw.githubusercontent.com/${username}/${repository}/main/week${weekNumber}.md`)
            ...
            .then(output => {
                ...
                const contentDiv = document.querySelector(`.${elementClass}`);
    ...
    loadWeekMarkdown(1, 'weekly-nerd-1');
...
// Voeg hier andere event listeners toe voor de andere gallery items
...
function closeTestScreen() {
    ...
    // Voeg hier andere class verwijderingen toe voor de andere weekly-nerd divs
}
...
document.addEventListener("DOMContentLoaded", function() {
    const container = document.querySelector(".vakkenReflectie");
    const bubbles = [
    ...
    bubbles.forEach((bubbleContent, index) => {
        ...
        bubble.style.left = `${(index % 3) * 33 + 10}%`;  
        bubble.style.top = `${Math.floor(index / 3) * 35 + 10}%`;  
